...
class AdaptLayer(BaseModule):
    ...
    def forward(self, hooked_tensors: Dict[str, Any], **kwargs) -> dict:
        ...
        if isinstance(self._id, Sequence):
            if self._multilevel:
                assert all(
                    len(self._id) == len(level_tensors)
                    for level_tensors in adapted_tensors
                )
...
class AdaptModuleList(ModuleList):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        adapts = [self.LAYER_TYPE.build(adapt) for adapt in adapts]
        super().__init__(modules=adapts, **kwargs)

    def forward(
        self,
        hooked_tensors: Dict[str, Any],
        inplace: bool = False,
    ) -> Dict[str, Any]:
        if not inplace:
